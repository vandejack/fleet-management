...
export const AlertsList = ({ alerts, vehicles = [], onDismiss, onClearAll }: AlertsListProps) => {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[1000] w-[340px] flex flex-col gap-3 pointer-events-none font-sans">
      {onClearAll && alerts.length > 0 && (
        <div className="pointer-events-auto flex justify-end mb-1">
          <button
            onClick={onClearAll}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-white/60 dark:bg-slate-800/60 hover:bg-white/80 dark:hover:bg-slate-700/60 text-slate-600 dark:text-slate-300 border border-white/20 dark:border-white/10 rounded-full text-[11px] font-medium transition-all backdrop-blur-xl shadow-sm hover:shadow-md"
          >
            <Trash2 size={12} />
              ...
              <div className="flex-1 min-w-0 pt-0.5">
                ...
                {driverName && (
                  <div className="flex items-center gap-1.5 mt-2">
                    <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-black/5 dark:bg-white/10 text-[10px] font-medium text-slate-600 dark:text-slate-300 border border-black/5 dark:border-white/5">
                      <User size={10} />
