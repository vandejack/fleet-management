              ...
              <div className="flex-1 overflow-y-auto p-2 space-y-2 scrollbar-thin scrollbar-thumb-slate-700">
                {vehicles.map((vehicle) => (
                  <div
                    ...
                    className={`group p-3 rounded-lg transition-all cursor-pointer border ${selectedVehicle?.id === vehicle.id
                  ...
                  >
                    <div className="flex justify-between items-center mb-1">
                      <span className={`font-bold transition-colors ${selectedVehicle?.id === vehicle.id ? 'text-cyan-400' : 'text-slate-200 group-hover:text-cyan-400'
                        }`}>{vehicle.name}</span>
            ...
            <GlassPanel className="h-48 p-4">
              ...
              <div className="h-full w-full pb-4">
                {isMounted ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center text-slate-500 text-[10px] font-mono">
                    CALIBRATING SYSTEMS...
                  </div>
                )}
              </div>
...
function NavItem({ icon, active, onClick }: { icon: React.ReactNode, active?: boolean, onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all duration-300 relative group ${active
...
function ActionRow({ icon, label, active }: { icon: React.ReactNode, label: string, active?: boolean }) {
  return (
    <div className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all ${active
